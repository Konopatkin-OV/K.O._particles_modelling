...
-- нельзя просто так сделать здесь флаг скрытия интерфейса, ибо при попытке его поменять
-- отваливаются приклеенные функции, завязанные на саму основу интерфейса
-- (отсутствие указателей = боль)
...
--
  ...
  , filename    :: String            -- имя файла, связанного с миром
  , is_active   :: Bool              -- флаг скрытия

  , edit_radius  :: Float            -- радиус круга добавления/удаления частиц
  , new_particle :: Entity           -- экземпляр новой частицы
  , action_type  :: Int              -- тип действия над частицами
  -- 0 - удаление, 1 - добавление
  -- TODO: 2 - ускорение/замедление, нужен параметр этого ускорения
  , m_act        :: Bool             -- захват мышью
  , m_pos        :: Point            -- положение указателя
  }
  deriving Show
